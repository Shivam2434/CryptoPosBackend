...
export class EthereumProvider extends BaseBlockchainProvider {
    ...
    constructor(private configService: ConfigService) {
        super();
        const rpcUrl = configService.get<string>('blockchain.ethereum.rpcUrl') || 'https://ethereum-rpc.publicnode.com';
        const sepoliaRpcUrl = configService.get<string>('blockchain.ethereum.sepoliaRpcUrl') || 'https://ethereum-sepolia-rpc.publicnode.com';

        this.provider = new ethers.JsonRpcProvider(rpcUrl);
        this.sepoliaProvider = new ethers.JsonRpcProvider(sepoliaRpcUrl);
        this.logger.log(`Ethereum providers initialized (Mainnet: ${rpcUrl}, Sepolia: ${sepoliaRpcUrl})`);
    }
    ...
    async getTransactionsForAddress(
        ...
        try {
            const currentBlock = await prov.getBlockNumber();
            const fromBlock = sinceBlock ? Math.max(sinceBlock, currentBlock - 50) : currentBlock - 20;
            ...
            const logs = await prov.getLogs(filter).catch((err) => {
                this.logger.debug(`Could not fetch logs for address ${address}: ${err.message}`);
                return [];
            });
            ...
            for (const log of logs) {
                ...
                const fromAddress = fromTopic ? ethers.stripZerosLeft(fromTopic) : 'unknown';
            ...
            const apiKey = this.configService.get<string>('blockchain.ethereum.apiKey');
