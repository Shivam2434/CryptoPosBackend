...
export class PairDeviceDto {
    @ApiProperty({ example: 'DEV-8F2A-91C0', description: 'Pairing code displayed in POS admin' })
    @IsString()
    @IsNotEmpty()
    deviceCode: string;

    @ApiPropertyOptional({ example: { model: 'Sunmi V2 Pro', serialNumber: 'SN98127391' } })
    @IsOptional()
    @IsObject()
