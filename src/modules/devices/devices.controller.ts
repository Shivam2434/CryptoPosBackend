...
export class DevicesController {
    ...
    @ApiBearerAuth()
    @ApiOperation({ summary: 'Register a new POS terminal/device and get pairing code' })
    register(@CurrentOrg() orgId: string, @Body() dto: RegisterDeviceDto) {
        return this.devicesService.register(orgId, dto);
    }

    @Post('pair')
    @ApiOperation({ summary: 'Pair device using pairing code (called by POS terminal device)' })
    ...
    findAll(@CurrentOrg() orgId: string, @Query('locationId') locationId?: string) {
