...
describe('WebhooksService', () => {
  ...
  beforeEach(async () => {
    endpointsRepo = {
      create: jest
        .fn()
        .mockImplementation((dto) => ({ id: 'ep-uuid-1', ...dto })),
    ...
    deliveriesRepo = {
      create: jest
        .fn()
        .mockImplementation((dto) => ({ id: 'del-uuid-1', ...dto })),
      save: jest.fn().mockImplementation((entity) => Promise.resolve(entity)),
      find: jest.fn(),
      findOne: jest.fn(),
    };
  ...
  it('should dispatch events to matching endpoints via queue', async () => {
    ...
    await service.dispatchEvent('org-uuid-1', 'payment.confirmed', {
      id: 'payment-1',
    });
