...
export class WebhooksController {
    ...
    createEndpoint(@CurrentOrg() orgId: string, @Body() dto: CreateWebhookEndpointDto) {
    ...
    findEndpointById(@CurrentOrg() orgId: string, @Param('id', ParseUUIDPipe) id: string) {
        return this.webhooksService.findEndpointById(orgId, id);
    }

    @Delete('endpoints/:id')
    @ApiOperation({ summary: 'Deactivate a webhook endpoint' })
    deleteEndpoint(@CurrentOrg() orgId: string, @Param('id', ParseUUIDPipe) id: string) {
