...
export class WebhooksService implements OnModuleInit {
  ...
  onModuleInit() {
    this.queueService.subscribe(
      'webhook-delivery',
      async (payload: { deliveryId: string }) => {
  ...
  async findEndpointById(
    ...
    if (!endpoint)
      throw new NotFoundException(`Webhook endpoint not found with ID: ${id}`);
    return endpoint;
  }
  ...
  async dispatchEvent(
    ...
    for (const ep of matching) {
      ...
      await this.queueService.publish('webhook-delivery', {
        deliveryId: saved.id,
      });
    }
  }
  ...
  async executeDelivery(deliveryId: string): Promise<void> {
    ...
    try {
      ...
      await this.deliveriesRepo.save(delivery);
      this.logger.log(
        `Webhook delivery ${delivery.id} (${delivery.eventType}) to ${endpoint.url} succeeded [${response.status}]`,
      );
    } catch (error) {
      ...
      if (delivery.attempts >= delivery.maxAttempts) {
        delivery.status = WebhookDeliveryStatus.FAILED;
        this.logger.warn(
          `Webhook delivery ${delivery.id} permanently failed after ${delivery.attempts} attempts`,
        );
      } else {
        const backoffSeconds = Math.min(
          60 * Math.pow(2, delivery.attempts - 1),
          7200,
        ); // 1m, 2m, 4m, 8m... max 2h
        ...
        this.logger.warn(
          `Webhook delivery ${delivery.id} failed attempt ${delivery.attempts}. Retrying in ${backoffSeconds}s`,
        );
      }
