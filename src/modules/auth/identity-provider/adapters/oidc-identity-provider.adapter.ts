...
export class OidcIdentityProviderAdapter implements IIdentityProvider {
  ...
  async resolveIdentity(token: string): Promise<VerifiedIdentity> {
    ...
    if (expectedAudience && payload.aud) {
      ...
      if (!audiences.includes(expectedAudience)) {
        throw new UnauthorizedException(
          `Token audience mismatch: expected ${expectedAudience}`,
        );
      }
    }

    // 4. Cryptographic signature verification via JWKS
    const isSignatureValid = await this.verifySignatureWithJwks(
      parts[0],
      parts[1],
      parts[2],
      header.kid,
    ...
    if (!isSignatureValid) {
      throw new UnauthorizedException(
        'Token cryptographic signature verification failed',
      );
    }
    ...
    const email =
      payload.email ||
      payload[`https://cryptopos.io/email`] ||
      `${sub}@idp.user`;
  ...
  private async getPublicKey(kid?: string): Promise<crypto.KeyObject | null> {
    ...
    if (
      ...
      Date.now() - this.lastJwksFetch > this.jwksCacheTtlMs
    ) {
  ...
  private async refreshJwks(): Promise<void> {
    ...
    if (!jwksUri) {
      this.logger.debug(
        'No JWKS URI or issuer URL configured for OIDC key retrieval',
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `Refreshed JWKS keys from ${jwksUri} (${this.jwksCache.size} keys imported)`,
      );
    } catch (error) {
      this.logger.warn(
