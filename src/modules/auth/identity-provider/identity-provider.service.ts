...
export class IdentityProviderService {
    ...
    getActiveAdapter(): IIdentityProvider {
        const providerName = (this.configService.get<string>('auth.provider') || 'oidc').toLowerCase();
        const adapter = this.adapters.get(providerName);
        if (!adapter) {
            this.logger.warn(`No specific adapter found for '${providerName}', falling back to OIDC adapter`);
