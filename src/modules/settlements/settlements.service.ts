...
export class SettlementsService {
    ...
    async createBatch(organizationId: string, dto: CreateSettlementBatchDto = {}): Promise<Settlement> {
        const query: any = {
            organizationId,
        ...
        if (payments.length === 0) {
            throw new BadRequestException('No confirmed, unsettled payments found for settlement');
        }

        // Calculate totals
        const grossAud = payments.reduce((sum, p) => sum + Number(p.audAmount), 0);
        const totalCrypto = payments.reduce((sum, p) => sum + Number(p.cryptoAmount), 0);
        ...
        const merchants = await this.merchantsService.findByOrganization(organizationId);
        const primaryMerchant = merchants[0];

        const providerName = dto.payoutProvider || 'manual_bank';
        const provider = providerName === 'mock' ? this.mockProvider : this.manualBankProvider;
        ...
        try {
            ...
            saved.status = result.status === 'completed' ? SettlementStatus.COMPLETED : SettlementStatus.PROCESSING;
            ...
            this.logger.error(`Payout execution failed for settlement ${saved.id}: ${error.message}`);
        ...
        this.webhooksService.dispatchEvent(organizationId, 'settlement.created', saved).catch(() => {});
    ...
    async markCompleted(organizationId: string, id: string, bankReference?: string): Promise<Settlement> {
