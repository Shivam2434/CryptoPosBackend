...
export class PricingService {
    ...
    async getAllPrices(
        ...
        for (const crypto of cryptos) {
            try {
                ...
                this.logger.error(`Failed to get price for ${crypto}`);
            }
        }
    ...
    async refreshPrices() {
        ...
        try {
            ...
            if (data.bitcoin?.aud) {
                this.priceCache['BTC_AUD'] = { price: data.bitcoin.aud, timestamp: now };
            }
            if (data.ethereum?.aud) {
                this.priceCache['ETH_AUD'] = { price: data.ethereum.aud, timestamp: now };
            }
            if (data.tether?.aud) {
                this.priceCache['USDT_ERC20_AUD'] = { price: data.tether.aud, timestamp: now };
            }
            if (data['usd-coin']?.aud) {
                this.priceCache['USDC_ERC20_AUD'] = { price: data['usd-coin'].aud, timestamp: now };
            }
