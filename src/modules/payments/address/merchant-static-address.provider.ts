...
export class MerchantStaticAddressProvider implements IAddressProvider {
    ...
    async allocateAddress(request: AddressAllocationRequest): Promise<AddressAllocationResult> {
        ...
        if (request.merchantId) {
            merchant = await this.merchantsService.findById(request.merchantId).catch(() => null);
        }

        if (!merchant) {
            const orgMerchants = await this.merchantsService.findByOrganization(request.organizationId);
            merchant = orgMerchants[0] || null;
        }

        if (!merchant) {
            throw new BadRequestException('No merchant profile found for organization to resolve receiving wallet');
        }
        ...
        if (!address) {
            throw new BadRequestException(`No wallet configured for ${request.cryptoType} under merchant ${merchant.businessName}`);
        }
