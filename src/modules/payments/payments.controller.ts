...
import {
    Controller, Post, Get, Body, Param, Query, UseGuards, Headers, ParseUUIDPipe,
} from '@nestjs/common';
import { ApiBearerAuth, ApiHeader, ApiTags, ApiOperation, ApiQuery } from '@nestjs/swagger';
import { CompositeAuthGuard } from '../../common/guards/composite-auth.guard';
import { CurrentAuth, CurrentOrg } from '../../common/decorators/current-tenant.decorator';
...
export class PaymentsController {
    ...
    @ApiBearerAuth()
    @ApiHeader({ name: 'x-api-key', required: false, description: 'Secret API key (sk_live_... or sk_test_...)' })
    @ApiHeader({ name: 'x-device-token', required: false, description: 'POS terminal device token' })
    @ApiHeader({ name: 'idempotency-key', required: false, description: 'Unique idempotency key to prevent double charge' })
    ...
    @Get(':id')
    @ApiOperation({ summary: 'Get payment details (Public - for customer checkout QR screen)' })
    getPayment(@Param('id', ParseUUIDPipe) id: string) {
        return this.paymentsService.getPayment(id);
    }

    @Get(':id/status')
    @ApiOperation({ summary: 'Get lightweight payment status (Public - for polling fallback)' })
