...
export class OrganizationsService {
    ...
    async getOrCreateForMerchant(merchant: { id: string; businessName: string; email: string; settlementPreference?: string }): Promise<Organization> {
        // Look up by slug or create
        const slug = this.generateSlug(merchant.businessName || `merchant-${merchant.id.substring(0, 8)}`);
