...
export class DeviceAuthGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        if (!deviceToken || typeof deviceToken !== 'string') {
            throw new UnauthorizedException('Device token is missing in X-Device-Token header');
        }

        try {
            const device = await this.devicesService.validateDeviceToken(deviceToken.trim());
