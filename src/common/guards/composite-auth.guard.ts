...
export class CompositeAuthGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        if (deviceTokenHeader) {
            return this.deviceGuard.canActivate(context) as Promise<boolean>;
        }

        // 2. If API Key header provided or Bearer contains sk_/pk_/cpk_
        if (apiKeyHeader || (authHeader && (authHeader.includes('sk_') || authHeader.includes('pk_') || authHeader.includes('cpk_')))) {
        ...
        throw new UnauthorizedException('Authentication credentials (Bearer token, X-Api-Key, or X-Device-Token) are required');
    }
}
